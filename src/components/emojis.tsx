...
export const Emojis = (msgHandler: any) => {
    ...
    React.useEffect(() => {
        if(msgHandler.channel.subscriptions.any.length === 0) {
            // make sure to add the listener only once
            console.log("Adding listener");
            msgHandler.channel.subscribe( (msg: any) => {
                if(msg.data.userId !== msgHandler.channel.messageHandler.userId) {
                    showRemoteEmoji(msg.data.message);
                    }
                })
            }
        });
    ...
    const getEmojiFromPicker = (emoji: any) => {
        ...
        msgHandler.channel.messageHandler.setMessage(emoji.native)
        msgHandler.channel.messageHandler.publishMessage()
        setEmojiComp(prevState => [...prevState ?? [], { id, flyingEmojiComp }]);
    }
    init({ data })
    return (   
